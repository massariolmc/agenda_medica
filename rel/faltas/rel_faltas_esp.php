...
include("../estilo.php");// NESSE ESTA A CONFIGURAÇÃO DO CABEÇALHO HTML E CSS        
        ...
        abreconexao();       
        $sql = "select p.nome_paciente as \"PACIENTE\", m.nome_med as \"MEDICO\", dt_consulta as \"DT\", hr_consulta as \"HORA\", c.especialidade as \"ESPECIALIDADE\"
                FROM consultas c
                INNER JOIN paciente p ON (status=4 AND p.cd_prontuario=c.cd_prontuario AND Extract('Month' FROM c.dt_consulta )=$mes AND Extract('Year' FROM c.dt_consulta )=$ano)
                INNER JOIN medico m ON(m.crm=c.crm AND m.cd_especialidade=c.especialidade AND c.especialidade='".$esp."')
                ORDER BY dt_consulta,p.nome_paciente";
        $query = pg_query($sql);    
        $cont = pg_affected_rows($query);       
        if($cont > 0){
        $art = array("consulta"  => array("PACIENTE","MEDICO","DATA", "HORA","ESPECIALIDADE"));
        $op = "consulta";
        $c = 2;
