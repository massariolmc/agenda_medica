...
<?php
	...
	if ( $pg > 0) { 
		echo "<a href=index.php?p=pagina&op=".$op."&pg=".($pg-1) ."class=pg><b>&laquo; anterior</b></a>";
	} else { 
	...
	for($i_pg=1;$i_pg<$quant_pg;$i_pg++) { 
		...
		if ($pg == ($i_pg-1)) { 
			...
			$i_pg2 = $i_pg-1;
			echo "&nbsp;<a href=index.php?p=pagina&op=".$op."&pg=$i_pg2 class=pg><b>$i_pg</b></a>&nbsp;";
		}
	}
	
	// Verifica se esta na ultima página, se nao estiver ele libera o link para próxima
	if (($pg+2) < $quant_pg) { 
		echo "<a href=index.php?p=pagina&op=".$op."&pg=".($pg+1)." class=pg><b>próximo &raquo;</b></a>";
	} else { 
